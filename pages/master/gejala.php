...
$totalData = $gejalaModel->countGejala($keyword);
$totalPages = ceil($totalData / $limit);
$dataGejala = $gejalaModel->getGejalaPaginated($keyword, $limit, $offset);
?>
...
<div class="mb-6 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
    ...
    <div class="flex">
        <button onclick="document.getElementById('modalTambah').classList.remove('hidden')" class="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-xl font-medium flex items-center transition-colors shadow-sm">
            <i data-lucide="plus" class="w-4 h-4 mr-2"></i> Tambah Gejala
        </button>
    </div>
</div>
...
<div class="card p-4 mb-6">
    <form method="GET" action="index.php" class="flex flex-col md:flex-row gap-4">
        <input type="hidden" name="page" value="gejala">
        <div class="flex-1">
            <div class="relative">
                <i data-lucide="search" class="w-5 h-5 absolute left-3 top-2.5 text-slate-400"></i>
                <input type="text" name="q" value="<?= htmlspecialchars($keyword) ?>" placeholder="Cari nama gejala atau kode..." class="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all">
            </div>
        </div>
        ...
        <a href="index.php?page=gejala" class="px-6 py-2 text-slate-500 font-medium hover:text-slate-800 transition-colors flex items-center">
            Reset
        </a>
        <?php endif; ?>
    </form>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Gejala</th>
                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right w-24">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                <?php foreach($dataGejala as $row): ?>
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="px-6 py-4 text-sm align-top">
                        <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded-md text-xs font-bold border border-blue-200"><?= $row['kode_gejala'] ?></span>
                    </td>
                    <td class="px-6 py-4 text-sm font-semibold text-slate-700 align-top"><?= htmlspecialchars($row['nama_gejala']) ?></td>
                    <td class="px-6 py-4 text-sm text-right align-top">
                        <button onclick="editGejala(<?= $row['id_gejala'] ?>, '<?= $row['kode_gejala'] ?>', <?= htmlspecialchars(json_encode($row['nama_gejala']), ENT_QUOTES) ?>)" class="text-blue-600 hover:text-blue-800 p-1.5 rounded-md hover:bg-blue-50 transition-colors" title="Ubah Data">
                            <i data-lucide="edit-3" class="w-4 h-4"></i>
                        </button>
                        <button onclick="confirmHapus(<?= $row['id_gejala'] ?>)" class="text-red-600 hover:text-red-800 p-1.5 rounded-md hover:bg-red-50 transition-colors ml-1 inline-block" title="Hapus Data">
                ...
                <?php if(count($dataGejala) == 0): ?>
                <tr>
                    <td colspan="3" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center text-slate-400">
                            <i data-lucide="activity" class="w-12 h-12 mb-3 opacity-20"></i>
                            <p class="text-slate-500 font-medium">Tidak ada data gejala ditemukan.</p>
                        </div>
                    </td>
                </tr>
                <?php endif; ?>
...
<div class="flex justify-between items-center mb-8">
    <p class="text-sm text-slate-500 font-medium">Menampilkan <?= count($dataGejala) ?> dari total <?= $totalData ?> data.</p>
    <div class="flex space-x-1">
        <?php for($i=1; $i<=$totalPages; $i++): ?>
            <?php 
                ...
                $url = "index.php?page=gejala&p=$i&q=".urlencode($keyword);
            ?>
...
<div id="modalTambah" class="fixed inset-0 bg-slate-900/50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-slate-800">Tambah Data Gejala</h3>
        ...
        <form method="POST">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="kode" value="<?= $gejalaModel->getNextKode() ?>" readonly class="w-full border border-slate-300 rounded-xl px-4 py-2 outline-none bg-slate-100 text-slate-500 cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Nama Gejala</label>
                    <textarea name="nama" rows="3" required class="w-full border border-slate-300 rounded-xl px-4 py-2 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500"></textarea>
                </div>
            </div>
...
<div id="modalEdit" class="fixed inset-0 bg-slate-900/50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-slate-800">Ubah Data Gejala</h3>
        ...
        <form method="POST">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Kode Gejala</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Nama Gejala</label>
                    <textarea name="nama" id="edit_nama" rows="3" required class="w-full border border-slate-300 rounded-xl px-4 py-2 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500"></textarea>
                </div>
            </div>
            <div class="mt-6 flex justify-end gap-3">
...
<script>
    ...
    function confirmHapus(id) {
        Swal.fire({
            ...
            text: "Data gejala yang dihapus tidak bisa dikembalikan!",
            icon: 'warning',
